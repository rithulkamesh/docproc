...
| [ARCHITECTURE.md](ARCHITECTURE.md) | **Architecture overview** — Pipeline flow, modules, Python library (`Docproc`). File in → markdown out; the full-stack demo lives in `demo/`. Examples: [examples/](../examples/). |
...
- **docproc (CLI or library)** — Document processor. Reads a file (PDF, DOCX, PPTX, XLSX), extracts text (native + optional vision for images), optionally refines with an LLM, and produces markdown (CLI writes a file; Python API returns a string or writes via `Docproc.extract_to_file`). No server, no database, no RAG in the core package.
